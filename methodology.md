...
- **Prices, dividends, splits.** Alpha Vantage `TIME_SERIES_DAILY_ADJUSTED` (premium). Raw close, adjusted close, dividend amount, and split coefficient are all stored so both return constructions in 8.1 can be computed and reconciled.
- **Shares outstanding.** Alpha Vantage `OVERVIEW` `SharesOutstanding`, refreshed at each quarterly reference date from that date forward (point-in-time from first production quarter). 
