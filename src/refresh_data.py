...
In CI:     the workflow sets ALPACA_API_KEY_ID and ALPACA_API_SECRET_KEY
           from repo secrets.
"""
...
def main():
    ...
    if key_id:
        cif.ALPACA_API_KEY_ID = key_id
    if secret:
        cif.ALPACA_API_SECRET_KEY = secret
    if (cif.ALPACA_API_KEY_ID.startswith("PUT_YOUR")
            or cif.ALPACA_API_SECRET_KEY.startswith("PUT_YOUR")):
        raise SystemExit("No Alpaca credentials. Set ALPACA_API_KEY_ID and "
